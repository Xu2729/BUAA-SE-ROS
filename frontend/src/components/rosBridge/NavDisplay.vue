...
import { onMounted, withDefaults, defineProps, watch, ref } from 'vue';
...
function drawPosition(
  to: { x: number; y: number; theta: number; name: string }[]
) {
  to.forEach((ele) => {
    var robotMarker = new ROS2D.NavigationArrow({
      size: 8,
      strokeSize: 1.2,
      fillColor: createjs.Graphics.getRGB(0, 128, 5, 0.66),
    });
    robotMarker.x = ele.x;
    robotMarker.y = -ele.y;
    robotMarker.scaleX = 1.0 / viewer2D.scene.scaleX;
    robotMarker.scaleY = 1.0 / viewer2D.scene.scaleY;
    robotMarker.rotation = (-ele.theta * 180) / Math.PI;
